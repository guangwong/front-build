...
var commands = module.exports = {
    ...
    build: function(bin, callback) {
        ...
        App.getApp(bin.cwd, function (err, app) {
            ...
            if (!app) {
                callback(new Error('build fail, not a fb app'));
                return;
            }
            ...
            async.series([
                ...
                function (callback) {
                    if (pages.length) {
                        app.buildPages(pages, argv, function (err) {
                            if (err) {
                                callback(err);
                                return;
                            }
                            ...
                            if (argv.watch) {
                                ...
                                app.startAutoBuildPages(pages, argv);

                                app.on('build_error', function (ev) {

                                    console.log('Build Fail(%s, %s)', ev.page.name, ev.page.version);
                                    errorHelper.printError(ev.error);
                                    watching();

                                });

                                app.on('watch_error', function (err) {
                                    errorHelper.printError(err);
                                    watching();
                                });

                                app.on('build_success', function (ev) {
                                    var page = ev.page;

                                    var out = util.format('\nbuild %s/%s to %s %s', page.name, page.version, page.timestamp, getTime().grey);
                                    console.log(out.underline);
                                    watching();
                                });
                                ...
                                function watching () {
                                    console.log('watching...'.cyan);
                                    if (argv.type) {
                                        console.log('build type: %s', argv.type);
                                    }
        ...
        App.getApp(cwd, function (err, app) {
            ...
            switch (command) {
                ...
                case 'add':
                    ...
                    if (argv._.length < 4) {
                        callback(new Error('set group with no page'));
                        return;
                    }
                    ...
                    app.getGroup(groupName, function (err, prev_pages) {
                        if (err) {
                            app.setGroup(groupName, pages, callback);
                            return;
                        }
                ...
                case 'build':
                    ...
                    app.getGroup(groupName, function (err, pages) {
                        ...
                        bin.argv._ = bin.argv._.slice(0,1).concat(pages);

                        commands.build(bin, callback);
                    });
