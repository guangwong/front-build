...
_.extend(App, {
    ...
    getGlobalConfig: function (callback) {
        ...
        var jsonpath = path.resolve(fu.getUserHome(), jsonFileName);
...
_.extend(App.prototype, {
    ...
    _copyFile: function(callback) {
        ...
        var fbRoot = path.resolve(__dirname, '..');
        var jobs = [
            {   
                filename: 'common/package-config.js',
                src: path.resolve(fbRoot, 'files/package-config.js'),
                dest: path.resolve(commonDir, 'package-config.js')
            },
            {
                filename: 'tools/web-client.sh',
                src: path.resolve(fbRoot, 'files/web-client.sh'),
                dest: path.resolve(self.rootDir, 'tools/web-client.sh')
            },
            {
                filename: 'tools/web-client.bat',
                src: path.resolve(fbRoot, 'files/web-client.bat'),
                dest: path.resolve(self.rootDir, 'tools/web-client.bat')
            },
            {
                filename: 'tools/app-update.bat',
                src: path.resolve(fbRoot, 'files/app-update.bat'),
                dest: path.resolve(self.rootDir, 'tools/app-update.bat')
            },
            {
                filename: 'tools/app-update.sh',
                src: path.resolve(fbRoot, 'files/app-update.sh'),
                dest: path.resolve(self.rootDir, 'tools/app-update.sh')
            }
        ];

        async.forEach(jobs, function (job, callback) {
            fs.exists(job.dest, function (exist){

                if (exist) {
                    console.log('file %s is already exist;', job.filename);
                    return callback(null);
                }

                var ws = fs.createWriteStream(job.dest, {start: 0});
                var rs = fs.createReadStream(job.src);
                rs.on('end', function (err) {
                    console.log('file %s added', job.filename);
                    callback(null);
                });
    ...
    getConfig: function (callback) {
        ...
        App.getGlobalConfig(function (globalConfig) {
            fu.readJSON(path.resolve(self.rootDir, App.FB_JSON_NAME), function (err, json) {
                if (err) {
                    return callback(err);
                }
                self.config = _.defaults(json, globalConfig);
                callback(null, json);
            });
        });
    },
