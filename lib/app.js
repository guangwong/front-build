...
_.extend(App.prototype, {
    ...
    update: function(dir, callback) {
        ...
        async.series([
            ...
            function (callback) {
                if (!self.config.version || self.config.version < App.version) {
                    self.config.version = App.version;
                    self.config = _.defaults(self.config, App.defaultConfig);
    ...
    buildCommon: function (callback){
        ...
        var commonPage = {
            ...
            inputCharset: self.config.charset,
            outputCharset: self.config.charset
        };
        ...
        async.series([
            function (callback){
                ...
                async.forEach([commonPage.srcDir, commonPage.destDir], function (absdir, callback){
                    path.exist(absdir, function(exist) {
                        if (exist) {
            ...
            function (callback) {
                //编码转换
                fu.iconvDir(commonDir, self.config.charset, commonPage.srcDir, 'utf8');
