...
_.extend(App.prototype, events.EventEmitter.prototype, /** @lends App.prototype */{
    ...
    /**
     * get Queue async Object for multi page build
     * @returns {Queue}
     */
    getPageBuildQueue: function(){
        var self = this;

        if (!self._pageBuildQueue) {
            self._pageBuildQueue = async.queue(function(task, callback) {
                var page = task.page;
                ...
                page.build(config, function(err){
                    if (err) {
                        self.emit('build_error', {
                            page: page,
                            error: err
                        });
                        callback(null);
    ...
    /**
     * Start Common AutoBuild
     */
    autoBuildCommon: function () {
        var self = this;

        self.startWatchCommon();
        self.on('commonChange', function () {
    ...
    /**
     * start watch pages and auto rebuild
     * @param {Array} pages
     * @param config
     */
    startAutoBuildPages: function (pages, config) {
        var self = this;

        var tPages = pages.map(function(page) {
            var parsed = Page.parsePageVersion(page);
            return self.getPage(parsed.name, parsed.version);
        });

        tPages.forEach(function (page) {
            page.startWatch(config.type);

            page.on('watch_error', function (err) {
                self.emit('watch_error', err);
            });

            page.on('change', function(ev){
                console.log('change', ev.path, page.name, page.version);
                self.getPageBuildQueue().push({
                    page: page,
                    config: config
                });
            });
        ...
        self.on('utilsChange', function(ev) {
            console.log('utils Change %s', ev.path.green);
            tPages.forEach(function(page) {
                self.getPageBuildQueue().push({
                    page: page,
                    config: config
    ...
    /**
     * build pages
     * @param {Array} pages to build
     * @param {Object} cfg build Config
     * @param {Function} callback
     */
    buildPages: function(pages, cfg, callback) {
        var self = this;
    ...
    _buildCommon: function(task, callback) {
        ...
        var src_page_dir = path.join(src_temp_dir, 'common');
        var build_page_dir = path.join(build_temp_dir, 'common');
        ...
        async.series([
            //准备临时目录
            function (callback){
                async.forEachSeries([commonPage.srcDir, commonPage.destDir, src_page_dir, build_page_dir], function (absdir, callback){
     ...
     */
    buildGroup: function (groupName, cfg, callback) {
        var self = this;
        ...
        self.getGroup(groupName, function (err, pages) {
            ...
            self.buildPages(pages, cfg, callback);
