...
_.extend(Page.prototype, events.EventEmitter.prototype, {
    ...
    addVersion: function (version, callback) {
        ...
        async.series([
            ...
            function (callback) {
                //write sh file
                var absPath = path.resolve(versionDir, Page.BUILD_SH);
            ...
            function (callback) {
                ...
                var absPath = path.resolve(versionDir, Page.DEFAULT_JS);
            ...
            function (callback) {
                //write bat file
                var absPath = path.resolve(versionDir, Page.BUILD_BAT);
     ...
     * @param {String}  version Version of page
    ...
    setVersion: function (version, callback) {
        ...
        fs.exists(self.versionDir, function(exist){
            if (!exist) {
                var error = new Error();
                error.name = 'E_NOPAGE';
                error.message = util.format('Page %s/%s is not exist', self.name, version);
